...
class TestInputContext final : public fcitx::InputContext {
  public:
    explicit TestInputContext(fcitx::Instance* instance)
        : InputContext(instance->inputContextManager(), "test") {
        created();
    }
    ...
    void commitStringImpl(const std::string& text) override { committed_ += text; }
    void deleteSurroundingTextImpl(int, unsigned int) override {}
    void forwardKeyImpl(const fcitx::ForwardKeyEvent&) override {}
    void updatePreeditImpl() override { ++preeditUpdates_; }

    void resetRecorder() { preeditUpdates_ = 0; }
    unsigned int preeditUpdates() const { return preeditUpdates_; }
...
void setIsolatedStandardPaths() {
    ...
    std::ofstream(root / "config/fcitx5/conf/lotus.conf")
        << "[General]\n"
        << "Mode=Smooth\n"
        << "InputMethod=Telex\n";
